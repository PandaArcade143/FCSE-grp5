	...
	public void showMenu(HDBOfficer hdbofficer) {
		...
		while (true) {
			...
			case 5:
				...
				System.out.print("Select which project to reply inquiries for: ");
			...
			case 6:
				if (hdbofficer.getApprovedProjects().isEmpty()) {
					...
					System.out.println("Current Projects: ");
					hdbofficer.getApprovedProjects().forEach(p->System.out.println(p.getName()));
					System.out.print("Select which project to book applicants for: ");
					proj = scanner.nextLine();
					s = hdbofficer.getApprovedProjects().stream()
							.filter(p -> p.getName().equals(proj))
							.findFirst();
					if (!s.isPresent()) {
						System.out.println("No matching project with that name.");
					...
					while (true) {
						System.out.print("\nEnter applicant's NRIC or type 'back' to go back to menu: ");
						...
						if (!nric.matches("[ST]\\d{7}[A-Z]")) {
							...
							System.out.println("Invalid NRIC given, please try Eagain.");
						} else {
